      ...
      },
      {
        ...
        items: {
          ru: [
            ...
            "Исправлен выход из незавершенного анализа: приложение предупреждает перед остановкой и не подхватывает старое ожидание при новом запуске.",
            "Исправлены мелкие недочеты интерфейса и русской локализации перед релизом.",
            "Исправлены fallback-строки интерфейса: участки, где в публичной английской базе оставались русские формулировки, переведены на английский.",
            "Исправлено зависание Codex bridge: если Codex не подключился к скану или не запустил инструменты, приложение теперь завершает ожидание понятной ошибкой.",
            "Исправлен пустой экран при старте: интерфейс больше не ждет бесконечно ответ от IPC локали и показывает экран ошибки вместо пустого окна.",
          ],
          en: [
            ...
            "Fixed leaving an unfinished analysis: the app now warns before stopping it and does not reuse the old waiting state on a new launch.",
            "Fixed small interface and Russian localization polish before release.",
            "Fixed interface fallback strings so public English defaults no longer contain Russian copy.",
            "Fixed Codex bridge waiting: if Codex does not connect to a scan or does not start tools, the app now ends the wait with a clear error.",
            "Fixed a blank startup screen: the interface no longer waits forever for locale IPC and now shows a renderer error screen instead of an empty window.",
          ],
        },
      },
    ],
      ...
      },
      {
        ...
        items: {
          ...
          en: [
            ...
            "MCP bridge path discovery now checks the ToraSEO Dev profile so Claude Desktop and Codex can find local development app sessions.",
            "Development bridge liveness and Codex setup verification now use the shared .toraseo-bridge directory to avoid AppData access issues in sandboxed Codex sessions.",
            "Codex and Claude setup-check responses no longer send users to a generic Scan button; they now distinguish setup verification, analysis screens, and Skill-only chat fallback.",
          ],
        },
      },
      {
